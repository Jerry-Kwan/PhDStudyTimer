...
- 电脑解锁后，用户可以手动恢复工作会话
- 暂停期间，计时器停止，但会话不会结束
- 在暂停状态下，用户可以手动编辑当前工作会话的已记录时长，以便调整不准确的记录或添加离线工作时间
...
#### 2.2 历史记录查看
- 用户可以查看所有历史工作会话
- 用户可以提供一个日期时间段（用 DatePicker 组件），指定查看开始日期在该时间段内的所有会话，若无指定，则默认显示所有会话
- 需要显示每个会话的开始时间、结束时间、工作时长，界面允许滚动以防止内容过多；开始和结束时间写出年月日时分，工作时长写出小时分钟

#### 2.3 历史记录清除
- 在历史记录查看界面，每个会话的右侧提供一个删除按钮，点击后从数据库中删除该会话，更新界面；同时提供一个删除所有按钮，点击后从数据库中删除当前展示的会话，更新界面
- 清除前显示确认对话框，防止误操作
...
#### 2.1 数据模型
- **WorkSession**: 工作会话模型，包含开始时间、结束时间、时长等信息
